...
describe("Agrifunding", () => {

    var address1; 
    var address2; 
    var agrifundingContract;
  
    beforeEach(async function () {
      [address1, address2,  ...address1] = await ethers.getSigners();
  
      const Agrifunding = await ethers.getContractFactory("Agrifunding");
      agrifundingContract = await Agrifunding.deploy();
    ...
    describe("Request for funding", async function () {
      it("Start a project", async function () {
        ...
        const project = await agrifundingContract.connect(address1).createProject(minimumContribution,deadline,targetContribution,projectTitle,projectDesc)
      ...
      it("Get data", async function () {
        ...
        const minimumContribution=etherToWei('1');
        const deadline=dateToUNIX('2023-12-25');
        const targetContribution=etherToWei('100');
        const projectTitle='Testing title';
        const projectDesc='Testing description';
