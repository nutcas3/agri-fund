...
const AgriRaiserCard = ({props,pushWithdrawRequests}) => {

  const [btnLoader, setBtnLoader] = useState(false)
  const [amount, setAmount] = useState(0)
  const dispatch = useDispatch();
  const AgriFundingContract = useSelector(state=>state.fundingReducer.contract)
  ...
  const contributeAmount = (projectId,minContribution) =>{
    ...
    const onSuccess = () =>{
      setBtnLoader(false)
      setAmount(0)
      toastSuccess(`Successfully contributed ${amount} tlos`)
    }
    const onError = (message) =>{
      setBtnLoader(false)
      toastError(message)
    }
    contribute(AgriFundingContract,data,dispatch,onSuccess,onError)
  }
  ...
  const requestForWithdraw = (projectId) =>{
    ...
    const data = {
      description:`${amount} tlos requested for withdraw`,
      amount:contributionAmount,
      recipient:account,
      account:account
    }
  ...
  return (
    ...
    <div className="flex flex-col lg:flex-row">
      <div className="inner-card my-6 w-full lg:w-2/5">
        ...
        <p className="text-sm font-bold font-sans text-gray-600 ">{props.goalAmount} tlos </p>
      ...
      <div className="inner-card my-6 w-full lg:w-3/5">
        {
          ...
          <>
            ...
            <p className="text-sm text-red-600"> <span className="font-bold">NOTE : </span> Minimum contribution is {props.minContribution} tlos </p>
          </>
          :
          <>
            <p className="text-md font-bold font-sans text-gray">Contract balance</p>
            <p className="text-sm font-bold font-sans text-gray-600 ">{props.contractBalance} tlos </p>
