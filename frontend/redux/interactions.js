...
import AgriFunding from '../../artifacts/contracts/Agrifunding.sol/AgriFunding.json'
...
const agriFundingContractAddress = '0x93154E21678A642C9BFeD4acC152c82f1aFc0734';
...
// Connect with Agrifunding contract
export const loadAgriFundingContract = async (web3, dispatch) => {
  try {
    const Agrifunding = new web3.eth.Contract(AgriFunding.abi, agriFundingContractAddress);
    dispatch(actions.agriFundingContractLoaded(Agrifunding));
    return Agrifunding;
  } catch (error) {
    console.error("Error loading AgriFundingContract:", error);
    throw error;
  }
};

// Subscribe to AgriFunding events
export const subscribeAgriFundingEvents = async (AgrifundingContract, dispatch) => {
  try {
    // Add your event subscription logic here
    // ...
  } catch (error) {
    console.error("Error subscribing to AgriFunding events:", error);
    throw error;
  }
};

// Start fundraising project
export const startFundRaising = async (web3, AgrifundingContract, data, onSuccess, onError, dispatch) => {
  try {
    const { minimumContribution, deadline, targetContribution, projectTitle, projectDesc, account } = data;
    await AgrifundingContract.methods.createProject(minimumContribution, deadline, targetContribution, projectTitle, projectDesc).send({ from: account })
    ...
    console.error("Error starting fundraising project:", error);
    throw error;
  }
}
...
export const getAllFunding = async(AgrifundingContract,web3,dispatch) =>{
   
  const fundingProjectList = await AgrifundingContract.methods.returnAllProjects().call()
...
export const contribute = async(AgrifundingContract,data,dispatch,onSuccess,onError) =>{
  const {contractAddress,amount,account} = data;
  await AgrifundingContract.methods.contribute(contractAddress).send({from:account,value:amount})
  .on('receipt', function(receipt){
...
export const getMyContributionList = async(AgrifundingContract,account) =>{
  const getContributions = await AgrifundingContract.getPastEvents("ContributionReceived",{
